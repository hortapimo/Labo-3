...
fin=-1
inicio=0
time, CH1, CH2=plotear("datos/der300hz.csv",ax,ax2,fin=fin, inicio=inicio)
#%%
fin=-850
inicio=850
m,cov = curve_fit(ajuste,time[inicio:fin],CH2[inicio:fin])
ax2.plot(time, ajuste(time,m[0],m[1]), c="red", label=f"Ajuste:${m[0]:.2f}[mV/ms]\cdot x + {m[1]:.1f}mV$",zorder=10)
ax.grid(which="major")
